...
Shared reference for `orca-task-runner`/`orca-evaluate`/`orca-workflow-task`/`orca-workflow-epic`/`orca-workflow` so a spawn failure gets checked
...
| *(pointer row, not a new literal — see root cause)* | The two literals in the `#42` row above are no longer the wrapper's complete match set: `_ORCA_RETRY_SIGNATURE_RE` in `orca_call_with_retry.sh` was broadened (2026-08-07) to also match 4 case-insensitive keywords (`not running` / `could not connect` / `reconnect` / `bootstrap`) not listed as literals here. Pasting those keywords into this column as if they were observed literals would violate this table's own literal-substring convention (`## Adding a new row` below), so this row exists only to point readers at the authoritative source instead of duplicating it | Before concluding a failure is *not* issue #42-class just because its text isn't one of the two literals above, read `orca_call_with_retry.sh`'s header comment (the "Signature broadened" paragraph) or grep `_ORCA_RETRY_SIGNATURE_RE` directly for the current match set | #42 |
...
| *(literal substring not yet confirmed in this environment — see root cause)* | A spawned worker/evaluator terminal's MCP server (observed: Context7) shows an interactive login/consent prompt before the session becomes usable. The injected dispatch spec sits unprocessed until a human clears it — none of `orca-task-runner`/`orca-evaluate`/`orca-workflow-task`/`orca-workflow-epic`/`orca-workflow`'s spawn templates precondition MCP auth state before spawning, and appending an ad hoc "continue anonymously if a login prompt appears" sentence to the dispatch spec text did not reliably prevent it (observed: blocked 2 of 4 spawns in the same session even with that sentence present) — the modal renders before the spec is ever read, so spec-text instructions can't act on it in time | Recover: `orca terminal read` to confirm the modal is actually present, then `orca terminal send --terminal <handle> --enter --json` is not guaranteed correct for a modal (may need a different key) — resend Enter only after confirming via read what the modal expects; if unclear, escalate to a human rather than guessing at keys. Prevent: the four coordinator/worker skills with an MCP-auth precondition bullet (`orca-task-runner`/`orca-evaluate`/`orca-workflow-task`/`orca-workflow-epic` — `orca-workflow` the router has none) state it once per session in their §0/전제 sections (skill files, issue #60) instead of repeating it per dispatch spec — confirm auth is complete or the server is disabled for the worker profile *before* spawning, not after. Next occurrence: capture the modal's literal on-screen text via `terminal read` and upgrade this row to a proper `failure_signature` substring — this row stays no-signature until then | #60 |
| *(no signature captured yet — see root cause)* | An evaluator terminal (REPL, no scheduled `terminal read` per `orca-workflow-task` SKILL.md §1's by-design model — results arrive via relay or a directly-read report file, never a read of this terminal) went unresponsive mid-session and was inferred crashed via structured status ("dispatch status=failed" / "terminal status=exited") rather than any `terminal read` output — this term log has zero `recv` events by design, so no literal substring was captured this occurrence. Root cause of the crash itself is unconfirmed; even the exact command/JSON that produced the "failed"/"exited" status pair wasn't captured this occurrence | fresh evaluator terminal re-spawn + re-dispatch — this consumes no task-level retry budget (`assignments.jsonl`'s `retry` counter is untouched by a spawn-failure respawn; verified against MediCount#470's log, where the crash-respawn kept `round:2` without bumping `retry`) since it isn't a FAIL verdict. Next occurrence: before respawning, run `orca terminal show --terminal <handle> --json` and `orca orchestration worker-show --dispatch <dispatch_id> --json`, capture the raw JSON verbatim, and attempt one `orca terminal read` even if expected empty — upgrade this row to a literal substring once actually observed | #61 |
| `is dispatched; only ready tasks can be dispatched` (full message: `Task <id> is dispatched; only ready tasks can be dispatched`) | attempting to re-`dispatch --task` a task that is already in `dispatched`/`completed` status — `dispatch` requires `status: ready` and carries no content-override argument | do not reuse a task_id across rounds; `task-create` a new task per round instead (`orca-workflow-task` §1 round-2+ relay) | #64 |
| `already has an active dispatch` (full message: `Terminal <handle> already has an active dispatch (ctx_... for task <id>)`) | a terminal can hold at most one active (non-completed) dispatch at a time; dispatching a new task to it before the current one reaches `completed` is rejected outright | wait via `check --wait` (`orca-workflows/self-recovery.md`) for that dispatch's `worker_done` — receiving it via `check`/`check --wait` is itself proof the dispatch reached `completed` server-side (confirmed live: `completedAt` matches the message timestamp exactly), no separate `task-list` confirmation read needed — before dispatching the next round | #64 |
| `Starting MCP servers` garbled repaint frames in scrollback while the injected spec's prefix is absent from the tail, immediately after `dispatch --inject` into a **freshly created** codex REPL (composer holds a placeholder/partial fragment instead of the spec) | `dispatch --inject`'s bracketed-paste races codex's MCP-server boot — `terminal wait --for tui-idle` is satisfied *before* boot finishes (observed live: spinner still animating at tui-idle), and text injected in that window is partially or wholly dropped. Enter-only retry is a byte-identical no-op — there is no draft to submit — which distinguishes this from `#43` above (draft intact, Enter lost) | follow `dispatch-verify.md`'s Pre-dispatch boot-quiesce procedure: after tui-idle, delay `dispatch --inject` until a cursor-scoped `terminal read` shows 0 new lines (~12s). Verified 2026-08-08: same site that dropped 3 consecutive injections injected cleanly under this wait and completed the full `worker_done` round-trip (`ctx_a3b164b8eacc` → task completed, `provenance: worker_report`) | #84 |
| `Rejected worker_done ... The Dispatch capability is missing` (`dispatch_capability_invalid`) | Unconfirmed hypothesis, not yet verified live: hand-specified spec_text for a `worker_done` send recipe may override the inject preamble's own recipe, sending without the dispatch capability the receiving end expects. Observed once: the worker's own `orca_call_with_retry` retry was independently accepted 21s later (dispatch/task had already completed by then), so this occurrence self-settled without the coordinator's resend instruction — that resend produced a second, equally stale rejection (capability already revoked by then) | No confirmed fix yet — this row exists so grep-first finds it as "known but unconfirmed" instead of re-diagnosing from scratch. If the coordinator's own resend is unnecessary (dispatch already completing server-side), do not resend — wait instead. Next occurrence: capture the dispatching skill's own `spec_text` verbatim (to test the override hypothesis) alongside the usual `dispatch-show`/`worker-show` JSON, and upgrade this row's root-cause column from hypothesis to confirmed | #110 |
| `consumer_fenced` (returned by `check --wait`, `task-create`, or other `orca orchestration` calls made by a coordinator whose worker terminal is still alive) | The coordinator's own handle becomes unbound from the run it created — cause not yet isolated to a specific call sequence — while the worker terminal keeps running and the run's task list stays valid | ① `orca orchestration run-use --id <run_id> --from <handle> --json` to rebind the coordinator handle to its own run, then retry the original call. ② If `check --wait` itself keeps returning `consumer_fenced` after rebinding, fall back to polling `orca orchestration task-list --run <run_id> --json` for completion and verify the produced artifact's schema directly, instead of depending on `worker_done` delivery. Verified live (2026-08-08): rebind not attempted that occurrence, but the task-list-polling fallback alone recovered the round without further coordinator restarts. Distinct from `#83` below — there, `task-list --run <run_id>` itself returns `run_not_found` (the run is gone, not merely this handle's binding to it) | #88 |
| `dispatch-show --task <id>` returns `dispatch:null`, or the run created by this session is absent from `run-list` (`run_not_found` from `task-list --run <id>`) — typically noticed via a task-runner failure text of `No recipient or active Dispatch Run could be resolved.` that does **not** match `orca_call_with_retry.sh`'s `_ORCA_RETRY_SIGNATURE_RE` (correctly — retrying against a wiped run cannot succeed) | Orca app auto-update mid-session (observed: 1.4.175→1.4.176) can reset orchestration state at the **Run level**, not just cause the transient reconnect `#42` handles — every dispatch under that run (including already-completed earlier rounds) resets with it, not just the in-flight one | Do not retry — a wiped run cannot be recovered by retrying the same call, and widening `_ORCA_RETRY_SIGNATURE_RE` to match this text would just waste time retrying against a dead run (regression to avoid). Instead: create a new Run via `orca orchestration run-create`, recover any `.orca-orphaned-result-<task_id>.json` the orphan-fallback contract already wrote (per the dispatching skill's own orphan-fallback clause), and resume from that recovered payload in a freshly spawned terminal. Verified live (2026-08-08): this exact recovery sequence completed the round without losing round-1's already-passed work | #83 |
...
row is from reading a target terminal that `orca-task-runner`/`orca-evaluate`/`orca-workflow-task`/`orca-workflow-epic`/`orca-workflow` spawned.

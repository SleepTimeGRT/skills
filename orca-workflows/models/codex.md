...
| 모델 | 강점 | 용도 | effort |
|---|---|---|---|
| `gpt-5.6-sol` (Sol) | 최상위 추론 + 장문 recall(MRCR 91.5%, SWE-Bench Pro 64.6%). $5/$30 per 1M | 정확성이 결정적인 리뷰·분석: server 로직·schema/migration·크립토·RLS/auth. **놓치면 안 되는 리뷰엔 Sol** — 재현율 우선, 오탐 있음(2026-07 웹 리서치, CodeRabbit·BSWEN 방향 일치·수치 미확정) | xhigh (high=비용 floor) |
| `gpt-5.6-terra` (Terra) | Sol 근접(MRCR 89.6%, SWE-Bench Pro 63.4%, Coding Agent Index 77), 출력 절반가 $2.50/$15 | Routine tier 전반: 기능 구현·리팩터·디버깅·테스트(`model-selection.md` Routine 정의와 동일 범위). 구현 워커 기본값. **리뷰어로는 쓰지 않는다** — review 앙상블에 추가 시 오히려 정확도 하락(-8.6pt, CodeRabbit 실측) | medium |

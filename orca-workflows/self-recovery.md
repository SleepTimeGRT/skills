...
by a caller that can never reach this sub-branch — do not cite it as precedent for what this sub-branch
requires. `orca-workflow-task`'s two roles above independently adopted the same file-naming convention
for their own per-`task_id` sidecar, which is fine — `task_id` is unique Orca-wide, so the two mechanisms
never collide — but that choice is not because this sub-branch mandates it.) Any per-dispatch storage works as

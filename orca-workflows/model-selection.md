...
| Claude | `claude-opus-5` | **agy 쿼터 소진 시 fallback.** OSWorld 2.0(컴퓨터 사용) 70.6% vs GPT-5.6 Sol 62.6%(2026-07 웹 리서치, 같은 벤치 스위트라 직접 비교 가능) — 이미 High Risk tier에 배정돼 있어 별도 provider 도입 없이 재사용. 1M 토큰 네이티브 컨텍스트로 Gemini의 128k MRCR 실측치를 상회. effort는 `high`(§ Exclusion 원칙과 동일하게 이 축은 judgment가 아니라 실행·리포팅이므로 xhigh 불필요). **주의**: 위 Gemini의 83%는 OSWorld-**Verified**, 이 70.6%는 OSWorld **2.0**으로 벤치 스위트가 달라 직접 비교 불가 — "Opus 5가 Gemini보다 낫다"는 근거가 아니라 "Sol보다 낫다"는 근거로만 쓴다. BU Benchmark의 Claude 수치는 Opus **4.8**(74%) 기준이라 Opus 5 고유 수치는 미확인. |

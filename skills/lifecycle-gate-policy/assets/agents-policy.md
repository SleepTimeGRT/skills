...
| `scripts/premerge.sh` | right before squash merge | independent gitleaks rescan (`origin/main..HEAD`, catches commits that bypassed pre-commit) + full `pnpm verify` + e2e (if configured, skipped when the diff is entirely docs/config-only per `E2E_EXEMPT_REGEX`) + review for code changes | final gate |

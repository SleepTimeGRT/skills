...
def _bare_wrapped_call_line_numbers(text: str) -> list[int]:
    """Line numbers (1-indexed) where an orca terminal-create/task-create/task-list/dispatch call
    appears inside a ```bash fenced block without 'orca_call_with_retry' on the same or immediately

...
- `orca-workflows/` is not deployed via `scripts/deploy-skills.sh` — edits there go live on merge to `main` (symlink-tracks-main convention). `skills/*/SKILL.md` changes need no separate deploy step beyond the normal git workflow for this task.
